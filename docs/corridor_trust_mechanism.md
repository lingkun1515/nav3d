...
这里的 "corridor" 指**沿全局路径方向延伸的 3D 空间包络（AABB 包围盒）**，并非狭义的建筑物走廊。之所以叫 corridor：

- 它是沿全局路径的一个矩形"信任通道"——通道内的激光点如果在 OctoMap 中有对应体素，说明是全局规划器已知的静态几何，直接放过；通道外或 OctoMap 未知的点则保留原碰撞检测。
- 这个术语借用了运动规划领域的常见说法（safe corridor、trust corridor）：在全局路径上划出一个空间区域，内部约束被放宽。
- 虽然它被设计用来解决楼梯攀爬问题（平地路径不产生 corridor segment，只有 `dz/dxy > slope_threshold` 的坡道段才激活），但机制本身对任何全局路径上已知静态几何都适用。
